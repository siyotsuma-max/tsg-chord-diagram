// ============================================================
// TSG Basic Chord Diagram — Service Worker（オフライン対応）
// ============================================================
// 方針：ページ本体(index.html)は「ネット優先（4秒タイムアウト）→保存版」。
//       画像は「保存版優先＋裏で更新」、外部（フォント等）は「保存版優先」。
// CACHE の版名を上げるべき時：アイコン等「index.html以外のファイル」を追加・削除した時。
// ※ 外部JSライブラリは使っていない。音源ファイルも無い。
// ============================================================
const CACHE = 'tsg-chord-v1';
...
self.addEventListener('activate', (event) => {
  event.waitUntil((async () => {
    ...
    await Promise.all(keys
      .filter((k) => k.startsWith('tsg-chord-') && k !== CACHE)
